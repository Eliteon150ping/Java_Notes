...
public class ExceptionHandling {
        public static void main(String[] args) {
            ...
            try (Scanner scanner = new Scanner(System.in)) {   // The "Try" block is used when you want a user to enter an
                System.out.print("Enter a number: ");          // expected value like a number but if they enter anything
                                                               // else
                int number = scanner.nextInt();                // like a string etc. The following catch blocks will
                                                               // display
            ...
            catch (Exception e) {                               // This is used for any error you would'nt expect(Like a
                                                                // last
                // SAFETY NET                                   //  resort.)
...
3) ArithmeticException: Mainly when a user divides by zero or some form of impossible or undefined mathematical result
                        occurs
...
9) OutOfMemoryError: This error indicates that the Java Virtual Machine (JVM) has run out of memory to allocate new
                     objects.
                     This can be caused by memory leaks or by attempting to allocate a very large amount of memory.

10) InputMismatchException: When a user types anything but a numeric value or a value that cannot be parsed into a numeric
                            value
...
1) NoSuchElementException: Thrown by methods in java.util classes to indicate that the element being requested does not
                           exist.
